...
def merged():
    ...
    try:
        ...
        if user_id:
            try:
                ...
                detected_confidence = float(_classification_quiet.get("confidence", 0) or 0)
                detected_mood = _classification_quiet.get("mood") or _classification_quiet.get("emotion") or None
                # 🔹 DECIDE RESPONSE TYPE (HOW TO RESPOND)
    ...
    "content": (
        ...
        f"Response style for this message: {response_type}.\n\n"
        "Definitions:\n"
        "- validate: acknowledge feelings, no advice\n"
        "- listen: stay present, ask gently\n"
        "- reflect: offer insight or mirror\n"
        "- act: suggest one small optional action\n\n"
        f"User support preferences (soft bias): {', '.join(support_focus) or 'none'}.\n"
        "Apply this only as a gentle tone bias. Never mention it explicitly.\n\n"
        "Always end with:\n"
        "Mind Mirror: one reflective line\n"
        "Mission: one optional growth action"
    )
}
        ...
        # Decide intent
        intent = detect_intent(entry)
        # 🔹 LOAD USER SUPPORT FOCUS (SOFT PREFERENCE)
        support_focus = []
        ...
        if user_id:
            try:
                ...
                print("⚠ Could not load support_focus:", e)
