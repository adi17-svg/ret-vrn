...
#         # 🔊 save audio locally (same as before)
#         filename = f"{user_id or 'anon'}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.wav"
#         os.makedirs("audios", exist_ok=True)
#         path = os.path.join("audios", filename)
#         audio_file.save(path)
...
#         # 👉 request transcription ONLY (no diarization)
#         transcript_post = requests.post(
#             "https://api.assemblyai.com/v2/transcript",
#             headers={"authorization": os.getenv("ASSEMBLYAI_API_KEY")},
#             json={"audio_url": audio_url},  # 👈 no speaker_labels
#         )
#         transcript_id = transcript_post.json().get("id")
...
#                 "diarized": False,  # 👈 explicitly no diarization
#                 "streak": streak,
#                 "rewards": rewards,
#                 "message_rewards": message_rewards,
#                 "missions_completed": missions_completed,
...
#         # classify whole transcript as ONE user message
#         try:
...
#         gamified = generate_gamified_prompt(stage or last_stage, transcript_text, evolution=bool(evolution_msg))

#         try:
#             question = generate_reflective_question(transcript_text, reply_to=reply_to or None)
#         except TypeError:
#             question = generate_reflective_question(transcript_text, reply_to)

#         # (optional) you can add TTS for spiral here if you want later
...
#             "reason": classification.get("reason"),
#             "transcription": transcript_text,
#             "diarized": False,          # 👈 no diarization
#             "ask_speaker_pick": False,  # 👈 no multi-speaker UI
#             "streak": streak,
#             "rewards": rewards,
#             "message_rewards": message_rewards,
#             "missions_completed": missions_completed,
...
# # Most latest working code without diarization but assembly ai api
# from flask import Blueprint, request, jsonify, Response, current_app
# import json
# import os
# import traceback
...
#                     # 🔹 NEW: mark that user just completed a mission
...
#         # 🔹 NEW: if we are in quick stage mode, force spiral (not chat)
#         if user_id and quick_stage_mode:
#             intent = "spiral"

#         # 🔹 NEW: if user is asking for their stage & we have no stage yet → ask simple question
...
#             # 🔹 NEW: soft B-type follow-up after mission completion (text + voice mode)
#             if user_id:
#                 try:
#                     progress = get_user_progress(user_id)
...
#         # 🔹 NEW: if this was quick-stage answer, save assessed_stage once
...
#                     # 🔹 NEW: mark mission completion for voice flow as well
#                     progress["just_completed_mission"] = True
...
#             # 🔹 NEW: same B-type follow-up after mission completion (voice input)
#             if user_id:
#                 try:
#                     progress = get_user_progress(user_id)
#                     if progress.get("just_completed_mission"):
...
#         # 🔹 if this was quick-stage answer, save assessed_stage once (voice)
#         if user_id and quick_stage_mode and stage:
#             try:
#                 progress = get_user_progress(user_id)
#                 progress["assessed_stage"] = stage
...
# #         # ✅ Mission tracking if replying (audio) — unchanged
# #         if reply_to and user_id:
# #             try:
# #                 with open("daily_tasks.json") as f:
# #                     tasks = json.load(f)
...
# #         # 🔹 NEW: handle ask_stage for voice when no stage yet
# #         if intent == "ask_stage" and user_id and (not has_stage) and (not quick_stage_mode):
# #             question_text = (
# #     "Love that you’re exploring your Spiral Dynamics level 💫 "
# #     "It shows you really care about understanding yourself.\n\n"
...
# #         # 🔹 NEW: if we are in quick stage mode, force spiral path
# #         if user_id and quick_stage_mode:
# #             intent = "spiral"
...
# #         # 🔹 NEW: if this was quick-stage answer, save assessed_stage once (voice)
# #         if user_id and quick_stage_mode and stage:
# #             try:
# #                 progress = get_user_progress(user_id)
# #                 progress["assessed_stage"] = stage
...
# #             with open(path, "rb") as f:
# #                 transcript = client.audio.transcriptions.create(
# #                     # You can use "whisper-1" too; this uses the newer model
# #                     model="gpt-4o-transcribe",
# #                     file=f,
# #                 )
# #             # Compatible access for both obj / dict style
# #             transcript_text = getattr(transcript, "text", "") or (
# #                 transcript.get("text", "") if isinstance(transcript, dict) else ""
# #             )
# #             transcript_text = (transcript_text or "").strip()
...
# #         # (optional) you can add TTS for spiral here later if you want
