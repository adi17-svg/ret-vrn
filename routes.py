...
def merged():
    try:
        ...
        # Load user progress
        # -------------------------
        user_progress = get_user_progress(user_id) if user_id else {}
        support_focus = user_progress.get("support_focus", [])
        support_focus_text = (
            " and ".join(support_focus)
            if isinstance(support_focus, list) and support_focus
            else "what you’re currently working through"
        )
        ...
        if intent == "chat":
            resp = client.chat.completions.create(
                model="gpt-4.1",
                messages=[
                    {"role": "system", "content": "You are a warm, supportive friend."},
        ...
        context_text = ""
        if user_id:
            recent = get_recent_conversation(user_id, limit=3)
            context_text = "\n".join([m["content"] for m in recent])

        # Classify spiral stage
        classification = classify_stage(entry, context=context_text)
        stage = classification.get("stage") or last_stage or "Unknown"
        confidence = classification.get("confidence", 1.0)
        mind_mirror = (
            classification.get("mind_mirror")
            or "there’s a subtle inner tension playing out right now"
        )

        # ---------------------------------------
        # Spiral lens wording (CONFIDENCE BASED)
        # ---------------------------------------
        if confidence >= 0.75:
            spiral_lens_line = f"From a {stage} lens, this response makes sense."
        elif confidence >= 0.55:
            spiral_lens_line = f"From a {stage}-leaning lens, this response makes sense."
        else:
            spiral_lens_line = "From a values-driven lens, this response makes sense."

        # ---------------------------------------
        # Inner pattern (derived from stage)
        # ---------------------------------------
        STAGE_PATTERNS = {
            "Green": "keeping harmony over expressing your own needs",
            "Blue": "doing what feels right over questioning expectations",
            "Orange": "pushing forward over slowing down",
            "Yellow": "seeing many perspectives over choosing one",
            "Red": "protecting control over vulnerability",
            "Purple": "seeking belonging over independence",
            "Beige": "seeking safety over reflection",
        }

        inner_pattern = STAGE_PATTERNS.get(
            stage, "responding automatically rather than intentionally"
        )

        # ---------------------------------------
        # Gentle experiment (generic & safe)
        # ---------------------------------------
        gentle_experiment = "notice one small moment where you pause before reacting"

        # Reflective question
        question = generate_reflective_question(entry)

        # =====================================================
        # 🔥 FINAL SPIRAL MESSAGE (THIS IS THE KEY CHANGE)
        # =====================================================
        spiral_message = (
            f"Since you’re working on {support_focus_text}, this pattern matters.\n"
            f"Right now, your mind is prioritizing {inner_pattern}.\n"
            f"{spiral_lens_line}\n"
            f"Mind mirror for this moment: {mind_mirror}.\n"
            f"A gentle experiment today: {gentle_experiment}.\n"
            f"{question}"
        )

        # Save assistant message
        if user_id:
            save_conversation_message(user_id, "assistant", spiral_message)

        return jsonify({
            "mode": "spiral",
            "response": spiral_message,
            "stage": stage,
