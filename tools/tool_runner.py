...
- NO Firestore writes
- Stateless dispatcher
- Auto-loads last N tool messages for context
- Tool controls flow via `step`
"""

from firebase_utils import get_recent_conversation

HISTORY_LIMIT = 6


def load_tool_history(user_id: str | None, tool_id: str):
    """
    Load last N messages for this tool from Firestore.
    """
    if not user_id:
        return []
...
def run_tool(
    ...
    user_id   → used only for loading last N messages
    """

    # 🔥 Load last 6 messages for this tool
    history = load_tool_history(user_id, tool_id)
    ...
    if tool_id == "getting_going":
        from tools.low_mood.getting_going import handle
        return handle(step, user_text, history)
